...
export const useCharacterStore = defineStore("character", () => {
		...
		//@ts-ignore
		const level:ComputedRef<Level> = computed(() => Object.values(levelTable).reverse().find(level => level.xp <= xp.value).level);
		const xp = ref(1500);
		const lifeCurrent = ref(20);
		const lifeMod = ref(0)
		// @ts-ignore
		const lifeMax = computed(() => levelTable[level.value].life + (level.value * abilityScoresStore.EnduranceBase) + lifeMod.value);
		...
		const manaMod = ref(0)
		// @ts-ignore
		const manaMax = computed(() => levelTable[level.value].mana + (level.value * abilityScoresStore.ThoughtBase) + manaMod.value);
		const profBonus: ComputedRef<number> = computed(() => levelTable[level.value].profBonus)
		const defenseMod = ref(0);
		const defense = computed( () => 8 + abilityScoresStore.AgilityCurrent + defenseMod.value + profBonus.value + (inventory.equippedArmor?.defenseMod || 0))
