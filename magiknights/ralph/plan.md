...
[
  ...
  },
  {
    "id": 37,
    "category": "layout-reorganization",
    "description": "Remove Magi-Squire from Student tab and add dual-view NPC page with Monster/NPC and Magi-Squire modes",
    "steps": [
      "Read `src/views/PC/StudentView.vue`",
      ...
      "In `src/stores/sheetStore.js`, add a `npc_sheet_type` ref (string, default 'monster') with options: 'monster' and 'squire'. Add it to the dehydrate/hydrate cycle for persistence. Add it to store exports",
      "In NPCView.vue script section, add the import for MagiSquire: `import MagiSquire from '@/components/MagiSquire.vue';`",
      "In NPCView.vue template, add a dropdown selector ABOVE the existing npc-header NotchContainer. Create a new div with class 'npc-type-selector' containing: a label 'Sheet Type:', a <select> bound to `sheet.npc_sheet_type` with two options: {value:'monster', label:'Monster / NPC'} and {value:'squire', label:'Magi-Squire'}",
      "Wrap the entire existing NPC content (from the npc-header NotchContainer through the npc-notes section) in a `<template v-if=\"sheet.npc_sheet_type === 'monster'\">` block",
      "After that template block, add `<template v-else-if=\"sheet.npc_sheet_type === 'squire'\"><MagiSquire /></template>` to show the Magi-Squire component when squire mode is selected",
      "Add CSS for the npc-type-selector: display flex, align-items center, gap, padding, margin-bottom, and style the select to match the existing NPC page styling",
      "Run `npm run build` to verify no build errors",
      "Update ARCHITECTURAL_ANALYSIS.md to document: Magi-Squire removed from Student tab, NPC page now has dual-view (Monster/NPC vs Magi-Squire) controlled by npc_sheet_type dropdown"
    ],
    "context": "The Magi-Squire companion (src/components/MagiSquire.vue) is currently rendered at the bottom of StudentView.vue. However, Magi-Squires are separate NPC entities - they have their own stats, health blips, mana blips, armor values, and damage scaling independent of the player character. They belong on the NPC sheet where they can have a full dedicated view. The NPC page (NPCView.vue) currently only handles Monster/NPC stat blocks. By adding a dropdown at the top that switches between 'Monster / NPC' and 'Magi-Squire' views, the same NPC sheet page can serve double duty. When 'Monster / NPC' is selected, the existing full NPC stat block interface shows. When 'Magi-Squire' is selected, the MagiSquire component renders instead. The MagiSquire component is already self-contained with its own template, making this a straightforward conditional render. A new `npc_sheet_type` ref in the store persists which view mode the NPC sheet is in.",
    "passes": false
  }
]
```
