...
export const performD20Roll = async (
  ...
  if (advantage !== undefined) {
    ...
    if (effectsSource) {
      ...
      actionDieValue = EffectsCalculator.calculateActionDie(
        EffectsCalculator.getValidEffects(effectsSource, actionDieKeys, (effect) => effect.enabled),
      );
    } else {
      rollBonuses = effectsStore.getModifiedRollBonuses(rollBonusKeys).value;
  ...
  const isRestricted = actionDieValue === -2;
  const dieSize = isRestricted ? 10 : 20;
  let numDice = 1;
  if (isRestricted) {
    numDice = 2;
  } else {
    numDice = 1 + Math.abs(actionDieValue);
  }
