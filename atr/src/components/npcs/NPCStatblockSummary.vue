<template>
  ...
  <summary class="npc-statblock__summary" @click.prevent>
    ...
    <div class="npc-statblock__summary-column">
      ...
      <div v-if="ballisticAC > 0" class="npc-statblock__summary-item">
        <strong>{{ t('titles.ballistic-ac') }}:</strong>
        <span v-if="!editMode">{{ ballisticAC }}</span>
        <input v-else type="number" v-model.number="localNpc.ballisticAC" min="0"/>
      </div>
    </div>
...
const ballisticAC = computed(() =>
  store.getNpcModifiedValue(
    props.npc._id,
    props.npc.ballisticAC ?? 0,
    effectKeys['ballistic-armor-class'],
  ).value.final,
);
