...
const props = defineProps<{
  ...
  proficiencyModifier: ModifiedValue;
...
const rollArgs = computed(() => {
  ...
  if (proficiency.label === 'insanity-saving') {
    const characterLevel = useProgressionStore().getLevel;
    if (characterLevel > 0) {
      bonuses.push({ label: t('titles.level'), value: characterLevel });
    }
  }
