...
import { arrayToObject, objectToArray, indexedObjectToArray, arrayToIndexedObject } from '@/utility/objectify';
import { v4 as uuidv4 } from 'uuid';
import { getEntryById } from '@/utility/getEntryBy';
import { useSpellsStore, type SpellCastingProgression, type StandardSpellSource } from '../spells/spellsStore';
...
export const useProgressionStore = defineStore('progression', () => {
  ...
  const getHitDice: ComputedRef<Partial<HitDiceCollection>> = computed(() => {
    if (hitDice.value.mode === 'manual') {
      return Object.fromEntries(Object.entries(hitDice.value.total).filter(([_, value]) => value && value > 0));
    } else {
      ...
      return Object.fromEntries(
        Object.entries(allHitDice).filter(([_, value]) => value && value > 0)
      ) as Record<HitDieSize, number>;
    }
  });

  const getLevel = computed(() => classes.value.reduce((level, cls) => level + (cls.level || 0), 0));
  ...
  const getClassesSummary = computed(() => {
    const summary = classes.value.map(cls => `${cls.subclass} ${cls.name} ${cls.level}`)
      .join(',').trim();
  ...
  const getAncestrySummary = computed(() => {
    if(!ancestry.value.subrace) {
      ...
      return ancestry.value.subrace.toLowerCase().includes(ancestry.value.name.toLowerCase()) ? ancestry.value.subrace : `${ancestry.value.subrace} ${ancestry.value.name}`;
    }
  });
  ...
  const updateClass = (patch: Partial<ClassProgression>): void => {
    ...
    if(!cls) {
      const newClass = {...getEmptyClass(), ...patch};
      classes.value.push(newClass);
    } else {
      const patched = {...cls, ...patch};
      if(patched.hitDie && patched.level) {
        const size = parseInt(patched.hitDie.split('d')[1]);
        const amountOfDice = parseInt(patched.hitDie.split('d')[0]);
        const hitPoints = Array.from({ length: patched.level ?? 1 }, (_, i) => {
          if (hitPointsMode.value === 'average') {
            return (Math.ceil(size / 2) + 1) * amountOfDice;
          } else if (hitPointsMode.value === 'rolled') {
            const diceRoll = new DiceRoll(cls.hitDie);
            diceRoll.roll();
            return diceRoll.total;
          } else {
            return 0; // manual mode, will be set later
          }
        });
        patched.hitPoints = hitPoints;
      } else {
        patched.hitPoints = [];
      }
      Object.assign(cls, patched);
    }
  };

  const removeClassFeatures = (_id: string, classFeatures: boolean = true, subclassFeatures: boolean = true, aboveLevel?: number) => {
    ...
    if(classFeatures) {
      if(aboveLevel) {
        existing.featureIds.filter(id => id.level > aboveLevel).forEach(fid => featuresStore.remove(fid.id));
        existing.featureIds = existing.featureIds.filter(id => id.level <= aboveLevel);

        //Expired Features
        existing.featureIds.filter(id => id.expirantionLevel && id.expirantionLevel < existing.level+1).forEach(fid => featuresStore.remove(fid.id));
        existing.featureIds = existing.featureIds.filter(id => !id.expirantionLevel || id.expirantionLevel >= existing.level-1);
      } else {
    ...
    if(subclassFeatures) {
      if(aboveLevel) {
        existing.subclassFeatureIds.filter(id => id.level > aboveLevel).forEach(fid => featuresStore.remove(fid.id));
        existing.subclassFeatureIds = existing.subclassFeatureIds.filter(id => id.level <= aboveLevel);

        //Expired Features
        existing.subclassFeatureIds.filter(id => id.expirantionLevel && id.expirantionLevel < existing.level+1).forEach(fid => featuresStore.remove(fid.id));
        existing.subclassFeatureIds = existing.subclassFeatureIds.filter(id => !id.expirantionLevel || id.expirantionLevel >= existing.level-1);
      } else {
  ...
  const removeClassSpellSource = (_id: string) => {
    ...
    const source = spellsStore.sources.find(s => s._id === existing.spellSourceId) as StandardSpellSource | undefined;
    if(!source) return;
  ...
  const addFeaturesToClass = async (_id: string, startingLevel: number, endingLevel: number = startingLevel, classFeatures: boolean = true, subclassFeatures: boolean = true, onlyExpired: boolean = false) => {
    const existing = classes.value.find(cls => cls._id === _id);
    if(!existing) return;
    ...
    try {
      const classData = existing.compendiumData.class ? JSON.parse(existing.compendiumData.class) : {};
      const subclassData = existing.compendiumData.subclass ? JSON.parse(existing.compendiumData.subclass) : {};

      if(classFeatures) {
        for(let lvl = startingLevel; lvl <= endingLevel; lvl++) {
          await dropClassFeatures({features:classData, updatedClass:existing, levelOverride:lvl, updateSubclass:false, onlyExpired, cascade});
        }
      }
      if(subclassFeatures) {
        for(let lvl = startingLevel; lvl <= endingLevel; lvl++) {
          await dropClassFeatures({features:subclassData, updatedClass:existing, levelOverride:lvl, updateSubclass:true, onlyExpired, cascade});
        }
      }
    } catch (error) {
  ...
  const addFeaturesToAncestry = async (startingLevel: number, endingLevel: number = startingLevel, raceFeatures: boolean = true, subraceFeatures: boolean = true, onlyExpired: boolean = false, previousLevels?: number[]) => {
    try {
      const raceData = ancestry.value.compendiumData.race ? JSON.parse(ancestry.value.compendiumData.race) : {};
      const subraceData = ancestry.value.compendiumData.subrace ? JSON.parse(ancestry.value.compendiumData.subrace) : {};
      ...
      if(raceFeatures) {
        ...
        for(let lvl = startingLevel; lvl <= endingLevel; lvl++) {
          await dropAncestryFeatures({features:raceData, levelOverride:lvl, updateSubrace:false, onlyExpired, previousLevels, cascade});
        }
      }
      if(subraceFeatures) {
        const cascade = {
        ...
        for(let lvl = startingLevel; lvl <= endingLevel; lvl++) {
          await dropAncestryFeatures({features:subraceData, levelOverride:lvl, updateSubrace:true, onlyExpired, previousLevels, cascade});
        }
      }
    } catch (error) {
      console.warn("Failed to parse class features from compendium data", error);
  ...
  const removeAncestryFeatures = (aboveLevel: number) => {
    ...
    if(ancestry.value.featureIds.length) {
      ...
      ancestry.value.featureIds = ancestry.value.featureIds.filter(fid => fid.level <= aboveLevel);
    }
    if(ancestry.value.subraceFeatureIds.length) {
      const expiredFeatures = ancestry.value.subraceFeatureIds.filter(fid => fid.level > aboveLevel);
      expiredFeatures.forEach(fid => featuresStore.remove(fid.id));
      ancestry.value.subraceFeatureIds = ancestry.value.subraceFeatureIds.filter(fid => fid.level <= aboveLevel);
    }
  }
  ...
  const addFeaturesToBackground = async (startingLevel: number, endingLevel: number = startingLevel, onlyExpired: boolean = false, previousLevels?: number[]) => {
    try {
      const backgroundData = background.value.compendiumData ? JSON.parse(background.value.compendiumData) : {};

      for(let lvl = startingLevel; lvl <= endingLevel; lvl++) {
        await dropBackgroundFeatures({features:backgroundData, levelOverride:lvl, onlyExpired, previousLevels});
      }
  ...
  const removeBackgroundFeatures = (aboveLevel: number) => {
    ...
    if(background.value.featureIds.length) {
      ...
      expiredFeatures.forEach(fid => featuresStore.remove(fid.id));
      background.value.featureIds = background.value.featureIds.filter(fid => fid.level <= aboveLevel);
    }
  }
  ...
  const hydrate = (payload: ProgressionHydrate) => {
    const classesHydration = payload?.classes
      ? objectToArray(payload.classes).map(cls => ({
          ...
          subclassFeatureIds: indexedObjectToArray(cls.subclassFeatureIds as Record<string, FeatureId>),
        }))
      : classes.value;
    ...
    const raceHydration = payload?.ancestry
      ? {
          ...payload.ancestry,
          featureIds: indexedObjectToArray(payload.ancestry.featureIds as Record<string, FeatureId>),
          subraceFeatureIds: indexedObjectToArray(payload.ancestry.subraceFeatureIds as Record<string, FeatureId>),
        }
      : ancestry.value;
    ...
    const backgroundHydration = payload?.background
      ? {
          ...payload.background,
          featureIds: indexedObjectToArray(payload.background.featureIds as Record<string, FeatureId>),
        }
      : background.value;
