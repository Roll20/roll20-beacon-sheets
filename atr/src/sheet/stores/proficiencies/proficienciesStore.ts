...
export const proficiencyLevels: Record<string, (typeof proficiencyLevelsValues)[number]> = {
  untrained: 0,
  'half-proficient': 0.5,
  proficient: 1,
  expert: 2,
...
export const useProficienciesStore = defineStore('proficiencies', () => {
  ...
  const ranked: Ref<RankedProficiency[]> = ref([
    ...
    getEmptyProficiency('insanity-saving', 'wisdom', 'savings'),
  ...
  const getPassiveProficiency = (proficiency: RankedProficiency): ModifiedValue => {
    ...
    return useEffectsStore().getModifiedValue(
      ...
      effectKeys[`${proficiency.label}-passive`],
    );
  };
  ...
  const insanitySaving = computed(() => ranked.value.find((p) => p.label === 'insanity-saving'));
  ...
  const getProficiencyModifier = (proficiency: RankedProficiency): ModifiedValue => {
    ...
    const addCharacterLevel =
      proficiency.label === 'insanity-saving' ? useProgressionStore().getLevel : 0; //TODO: Refactor this, move into a effect add:@{level}

    const baseValue = rawAbilityBonus + proficiencyBonus + addCharacterLevel;

    const keysToQuery: string[] = [];
    keysToQuery.push(effectKeys[`${proficiency.label}`]);
    if (proficiency.group === 'savings') {
