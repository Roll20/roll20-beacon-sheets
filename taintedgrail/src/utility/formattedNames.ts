...
const FRIENDLY_NAME_TO_DOMAIN_MAP = Object.fromEntries(
  Object.entries(DOMAIN_NAME_MAP).map(([key, value]) => [value, key]),
) as Record<string, keyof typeof DOMAIN_NAME_MAP>;
...
export const friendlyNameToDomain = (friendlyName: string): string => {
  return (
    FRIENDLY_NAME_TO_DOMAIN_MAP[friendlyName] || friendlyName.toLowerCase().replace(/\s+/g, '')
  );
};

// Bidirectional mapping for spell disciplines
const SPELL_DISCIPLINE_NAME_MAP = {
  sorcery: 'Sorcery',
  wyrdMagic: 'Wyrd Magic',
  lowMagic: 'Low Magic',
  druidism: 'Druidism',
} as const;
...
export const spellDisciplineToFriendlyName = (discipline: string): string => {
  return (
    SPELL_DISCIPLINE_NAME_MAP[discipline as keyof typeof SPELL_DISCIPLINE_NAME_MAP] || discipline
  );
};

export const friendlyNameToSpellDiscipline = (friendlyName: string): string => {
  return (
    FRIENDLY_NAME_TO_SPELL_DISCIPLINE_MAP[friendlyName] ||
    friendlyName.toLowerCase().replace(/\s+/g, '')
  );
};
