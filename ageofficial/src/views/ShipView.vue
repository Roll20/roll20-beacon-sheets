      ...
      <div>
        ...
        <div class="age-ship-details ship-chip-strip">
          ...
          <div class="age-ship-chip">
            ...
            <span class="age-ship-chip-value">
              {{ ship.sensorsCurrent }}
              <span v-if="ship.losses.sensors > 0" class="age-ship-chip-base"
                >(base {{ ship.sensorsBase }})</span
              >
            </span>
          </div>
          <div class="age-ship-chip">
            ...
            <span class="age-ship-chip-value">
              <span v-for="drive in ship.drive" :key="drive._id">{{
                drive.type
              }}</span>
              <span
                v-if="ship.seriousLosses.reactorOffline"
                class="age-chip-offline"
                >OFFLINE</span
              >
            </span>
          </div>
        ...
        <div class="section-card ship-card-mb">
          ...
          <div class="age-ship-actions">
            ...
            <button class="age-btn" @click="onEvasion">
              Evasion<span v-if="highGBonusInput > 0" class="age-btn-badge"
                >+{{ highGBonusInput }}</span
              >
            </button>
            <button
              class="age-btn"
              @click="onPointDefense"
              v-tippy="{
                content: ship.pdcFiredThisRound
                  ? 'TN+2: PDCs already fired this round'
                  : 'Point defense test',
              }"
            >
              Point Defense<span
                v-if="ship.pdcFiredThisRound"
                class="age-btn-badge"
                >TN+2</span
              >
            </button>
          ...
          <button
            class="age-btn age-btn--dim ship-wide-btn"
            @click="ship.resetPdcFiredThisRound()"
            v-tippy="{
              content: 'Clear PDC-fired flag and begin a new combat round',
            }"
          >
            New Round
          </button>
      ...
      <div class="section-card ship-losses-card">
        <div class="ship-losses-normal">
          ...
          <div class="age-ship-losses">
            <div
              v-for="loss in shipLossTypes"
              :key="loss.key"
              class="age-ship-loss-row"
            >
              <span class="age-ship-loss-label">{{ loss.label }}</span>
              <button
                class="age-icon-btn"
                @click="ship.adjustLoss(loss.key, -1)"
                :disabled="ship.losses[loss.key] === 0"
              >
                −
              </button>
              <span
                class="age-ship-loss-value"
                :class="{ 'age-ship-loss-active': ship.losses[loss.key] > 0 }"
              >
              ...
              <button
                class="age-icon-btn"
                @click="ship.adjustLoss(loss.key, 1)"
                :disabled="ship.losses[loss.key] === 6"
              >
                +
              </button>
            </div>
        ...
        <div class="ship-losses-serious">
          ...
          <div class="age-ship-serious-losses">
            <label
              v-for="sl in seriousLossTypes"
              :key="sl.key"
              class="age-checkbox-toggle-row"
            >
              <input
                type="checkbox"
              ...
              />
              <span
                :class="{ 'age-ship-loss-active': ship.seriousLosses[sl.key] }"
                >{{ sl.label }}</span
              >
            </label>
          </div>
...
const onEvasion = () =>
  ship.rollEvasion(enemySensorsInput.value, highGBonusInput.value);
...
@media (max-width: 600px) {
  .ship-view {
    ...
    & .ship-config-btn:hover,
    & .ship-config-btn:focus {
      opacity: 1;
    }
    & .age-ship-chip-label {
      font-size: 1.35rem;
