...
export const maneuverabilityPenalty = computed(
  () => -useShipStore().losses.maneuverability
);
...
export const evasionPenalty = computed(() => {
  ...
  return (
    -ship.losses.maneuverability + (ship.seriousLosses.reactorOffline ? -2 : 0)
  );
});

// Whether High-G Maneuvers are allowed (blocked by Reactor Offline serious loss)
export const highGAllowed = computed(
  () => !useShipStore().seriousLosses.reactorOffline
);

// Per-stack penalty to weapon damage from weapons system loss
export const weaponsSystemDamageMod = computed(
  () => -useShipStore().losses.weaponsSystem
);
