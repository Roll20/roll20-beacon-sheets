...
export const useItemStore = defineStore("quality", () => {
  ...
  const focusItems = items.value.filter(
    (item) => item.type === "Ability Focus"
  );
  const focusItems2 = () => {
    return items.value.filter((item) => item.type === "Ability Focus");
  };
  const stuntItems = items.value.filter(
    (item) => item.type === "Favored Stunt"
  );
  const talentItems = items.value.filter((item) => item.type === "Talent");
  const specializationItems = items.value.filter(
    (item) => item.type === "Specialization"
  );
  const ancestryClassItems = items.value.filter(
    (item) => item.type === "Ancestry" || item.type === "Class"
  );
