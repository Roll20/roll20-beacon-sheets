...
export const useShipStore = defineStore("ship", () => {
  ...
  const shipStunts = ref<{
    commander: number;
    pilot: number;
    engineer: number;
    gunner: number;
    sensors: number;
  }>({ commander: 0, pilot: 0, engineer: 0, gunner: 0, sensors: 0 });
  ...
  const sensorsCurrent = computed(() =>
    Math.max(0, sensorsBase.value - losses.value.sensors)
  );
  ...
  const getCrewAbilityForRole = (
    roleKey: string
  ): { abilityValue: number; focusBonus: number } => {
    const focusName = roleFocusMap[roleKey];
    if (!focusName) return { abilityValue: 0, focusBonus: 0 };

    const getAbilities = (member: CrewHydrate) =>
      Array.isArray(member.ability)
        ? member.ability
        : objectToArray(member.ability || {});
    ...
    for (const member of crew.value) {
      for (const ab of getAbilities(member)) {
        if (ab.focusValue === focusName && ab.primary) {
          return {
            abilityValue: Number(ab.value) || 0,
            focusBonus: ab.doubleFocus ? 4 : ab.focus ? 2 : 0,
          };
        }
      }
    }
  ...
  const rollEvasion = async (enemySensors: number, highGBonus: number = 0) => {
    ...
    if (maneuverPenalty < 0)
      components.push({
        label: "Maneuverability Loss",
        value: maneuverPenalty,
      });
    if (reactorPenalty < 0)
      components.push({ label: "Reactor Offline", value: reactorPenalty });
    if (highGBonus > 0)
      components.push({ label: "High-G Maneuver", value: highGBonus });
    await rollToChat({
      characterName: useMetaStore().name,
      title: "Evasion",
      subtitle: `Dexterity (Piloting) vs TN ${tn}${
        highGBonus > 0
          ? ` — High-G +${highGBonus} (crew Con TN ${8 + highGBonus})`
          : ""
      }`,
      rollType: "standard",
      hideSuccessText: true,
  ...
  const rollPointDefense = async (enemySensors: number) => {
    ...
    await rollToChat({
      characterName: useMetaStore().name,
      title: "Point Defense",
      subtitle: `Sensors vs TN ${tn}${
        pdcPenalty > 0 ? " (+2 — PDCs already fired this round)" : ""
      }`,
      rollType: "standard",
      hideSuccessText: true,
  ...
  const rollDamageControl = async () => {
    ...
    await rollToChat({
      ...
      subtitle:
        "Intelligence (Engineering) vs TN 11 — ST 5 (1 loss) / ST 10 (2 losses)",
      rollType: "standard",
      hideSuccessText: true,
  ...
  const rollHull = async () => {
    ...
    await rollToChat({
      ...
      subtitle:
        hullFormula +
        (losses.value.hull > 0 ? ` − ${losses.value.hull} (Hull Loss)` : ""),
      rollType: "standard",
      hideSuccessText: true,
      traits: ["Ship", "Hull"],
      components,
  ...
  const resetShipStunts = (
    stuntType: "commander" | "pilot" | "engineer" | "gunner" | "sensors"
  ) => {
  ...
  const hydrate = (hydrateStore: ShipHydrate) => {
    ...
    crew.value =
      crewArray.map((c: any) => ({
        ...c,
        ability: Array.isArray(c.ability)
          ? c.ability
          : objectToArray(c.ability || {}),
      })) || [];
    ...
    shipStunts.value = hydrateStore.shipStunts || {
      commander: 0,
      pilot: 0,
      engineer: 0,
      gunner: 0,
    ...
    losses.value = hydrateStore.losses ?? {
      collateral: 0,
      hull: 0,
      maneuverability: 0,
      sensors: 0,
    ...
    seriousLosses.value = hydrateStore.seriousLosses ?? {
      reactorOffline: false,
      railgunsOffline: false,
      torpedoesOffline: false,
      pdcsOffline: false,
