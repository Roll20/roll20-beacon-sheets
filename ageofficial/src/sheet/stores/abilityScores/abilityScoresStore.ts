...
export const useAbilityScoreStore = defineStore("abilityScores", () => {
  ...
  const abilityScores = computed({
    get() {
      return {
        Accuracy: {
          base: AccuracyBase.value,
          current: AccuracyCurrent.value,
          partialAdvancement: AccuracyPartialAdvancement.value,
        },
        Communication: {
          base: CommunicationBase.value,
          current: CommunicationCurrent.value,
          partialAdvancement: CommunicationPartialAdvancement.value,
        },
        Constitution: {
          base: ConstitutionBase.value,
          current: ConstitutionCurrent.value,
          partialAdvancement: ConstitutionPartialAdvancement.value,
        },
        Dexterity: {
          base: DexterityBase.value,
          current: DexterityCurrent.value,
          partialAdvancement: DexterityPartialAdvancement.value,
        },
        Fighting: {
          base: FightingBase.value,
          current: FightingCurrent.value,
          partialAdvancement: FightingPartialAdvancement.value,
        },
        Intelligence: {
          base: IntelligenceBase.value,
          current: IntelligenceCurrent.value,
          partialAdvancement: IntelligencePartialAdvancement.value,
        },
        Perception: {
          base: PerceptionBase.value,
          current: PerceptionCurrent.value,
          partialAdvancement: PerceptionPartialAdvancement.value,
        },
        Strength: {
          base: StrengthBase.value,
          current: StrengthCurrent.value,
          partialAdvancement: StrengthPartialAdvancement.value,
        },
        Willpower: {
          base: WillpowerBase.value,
          current: WillpowerCurrent.value,
          partialAdvancement: WillpowerPartialAdvancement.value,
        },
      };
    },
    set(scores) {
      ...
      CommunicationBase.value =
        scores.Communication.base || CommunicationBase.value;
      ConstitutionBase.value =
        scores.Constitution.base || ConstitutionBase.value;
      DexterityBase.value = scores.Dexterity.base || DexterityBase.value;
      FightingBase.value = scores.Fighting.base || FightingBase.value;
      IntelligenceBase.value =
        scores.Intelligence.base || IntelligenceBase.value;
      ...
      CommunicationCurrent.value =
        scores.Communication.current || CommunicationCurrent.value;
      ConstitutionCurrent.value =
        scores.Constitution.current || ConstitutionCurrent.value;
      DexterityCurrent.value =
        scores.Dexterity.current || DexterityCurrent.value;
      FightingCurrent.value = scores.Fighting.current || FightingCurrent.value;
      IntelligenceCurrent.value =
        scores.Intelligence.current || IntelligenceCurrent.value;
      PerceptionCurrent.value =
        scores.Perception.current || PerceptionCurrent.value;
      StrengthCurrent.value = scores.Strength.current || StrengthCurrent.value;
      WillpowerCurrent.value =
        scores.Willpower.current || WillpowerCurrent.value;
    },
  });
  // Example for how to make basic roll + some modifiers using a custom roll template.
  const rollAbilityCheck = async (
    ...
    const modResult = mods.modifiers
      .filter(
        (mod) =>
          mod.option === "Ability" &&
          mod.modifiedValue === abilityScore &&
          !mod.abilityFocus &&
          mod.enabled
      )
      .reduce(function (acc, obj) {
        return acc + (obj.bonus || obj.penalty || 0);
      }, 0);
    ...
    const components = [
      ...
      {
        label: "Modifiers",
        value: (Number(abilityBonus) || 0) + (!proficient ? modResult : 0),
      },
    ];
  ...
  const rollAbilityInitiative = async (
    abilityScore: AbilityScore,
    proficient: boolean = false
  ) => {
    const score = abilityScores.value[abilityScore].base;
