...
export const useCustomConditionsStore = defineStore("customConditions", () => {
  ...
  const removeCondition = (_id: string) => {
    const indexToRemove = customConditions.value.findIndex(
      (customConditions) => customConditions._id === _id
    );
    const mods = useModifiersStore();
    const attachedMods = mods.parentItems(_id);
    if (attachedMods.length > 0)
      attachedMods.forEach((mod) => mods.removeModifier(mod._id));
    if (indexToRemove >= 0) customConditions.value.splice(indexToRemove, 1);
  };
  const showCondition = (_id: string) => {
    const indexToShow = customConditions.value.findIndex(
      (customConditions) => customConditions._id === _id
    );
    if (indexToShow >= 0)
      customConditions.value[indexToShow].show =
        !customConditions.value[indexToShow].show;
  };
  const getConditionById = (_id: string) => {
    const i = customConditions.value.findIndex(
      (customConditions) => customConditions._id === _id
    );
    if (i >= 0) customConditions.value[i];
  };
  const addModifier = (customCondition: any) => {
    const condition = customConditions.value.find(
      (qty) => qty._id === customCondition._id
    );
    if (!condition) return;
  ...
  const removeModifier = (_id: string, index: number) => {
    const condition = customConditions.value.find(
      (customCondition) => customCondition._id === _id
    );
    if (!condition) return;
  ...
  const hydrate = (hydrateStore: ConditionsHydrate) => {
    customConditions.value =
      objectToArray(hydrateStore.customConditions?.customConditions) ||
      customConditions.value;
