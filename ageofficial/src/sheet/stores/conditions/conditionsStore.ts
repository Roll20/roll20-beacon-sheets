...
export const useConditionsStore = defineStore("conditions", () => {
  ...
  const removeCondition = (_id: string) => {
    const indexToRemove = conditions.value.findIndex(
      (conditions) => conditions._id === _id
    );
    if (indexToRemove >= 0) conditions.value.splice(indexToRemove, 1);
  };
  const showCondition = (_id: string) => {
    const indexToShow = conditions.value.findIndex(
      (conditions) => conditions._id === _id
    );
    if (indexToShow >= 0)
      conditions.value[indexToShow].show = !conditions.value[indexToShow].show;
  };
  const dehydrate = () => {
    return {
      conditions: {
  ...
  const hydrate = (hydrateStore: ConditionsHydrate) => {
    conditions.value =
      objectToArray(hydrateStore.conditions?.conditions) || conditions.value;
