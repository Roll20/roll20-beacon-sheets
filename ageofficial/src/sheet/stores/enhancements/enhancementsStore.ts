...
export const useEnhancementStore = defineStore("enhancements", () => {
  ...
  const enhancementsCount: ComputedRef<number> = computed(
  ...
  const removeEnhancement = (_id: string) => {
    const indexToRemove = enhancements.value.findIndex(
      (enhancements) => enhancements._id === _id
    );
  ...
  const hydrate = (hydrateStore: EnhancementsHydrate) => {
    enhancements.value =
      objectToArray(hydrateStore.enhancements?.enhancements) ||
      enhancements.value;
  };
  return {
    enhancements,
