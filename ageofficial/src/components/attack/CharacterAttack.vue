  ...
  <div class="accordion-header age-combat-accordion-header">
    <div class="age-combat-icon age-flex">
      <div class="atk-icon-frame" v-if="attack.weaponType !== 'Natural'">
        <div
          class="age-combat-section age-combat-axes-icon"
          v-if="attack.weaponGroup === 'Axes'"
        ></div>
        <div
          class="age-combat-section age-combat-blackPowder-icon"
          v-if="attack.weaponGroup === 'Black Powder'"
        ></div>
        <div
          class="age-combat-section age-combat-bludgeons-icon"
          v-if="attack.weaponGroup === 'Bludgeons'"
        ></div>
        <div
          class="age-combat-section age-combat-bows-icon"
          v-if="
            attack.weaponGroup === 'Bows' ||
            (attack.option === 'Custom Attack' &&
              attack.weaponType === 'Ranged')
          "
        ></div>
        <div
          class="age-combat-section age-combat-brawling-icon"
          v-if="
            (settings.gameSystem !== 'expanse' &&
              attack.weaponGroup === 'Brawling') ||
            (attack.option === 'Custom Attack' && attack.weaponType === 'Melee')
          "
        ></div>
        <div
          class="age-combat-section age-combat-dueling-icon"
          v-if="attack.weaponGroup === 'Dueling'"
        ></div>
        <div
          class="age-combat-section age-combat-heavyBlades-icon"
          v-if="attack.weaponGroup === 'Heavy Blades'"
        ></div>
        <div
          class="age-combat-section age-combat-lances-icon"
          v-if="attack.weaponGroup === 'Lances'"
        ></div>
        <div
          class="age-combat-section age-combat-lightBlades-icon"
          v-if="attack.weaponGroup === 'Light Blades'"
        ></div>
        <div
          class="age-combat-section age-combat-polearms-icon"
          v-if="attack.weaponGroup === 'Polearms'"
        ></div>
        <div
          class="age-combat-section age-combat-slings-icon"
          v-if="attack.weaponGroup === 'Slings'"
        ></div>
        <div
          class="age-combat-section age-combat-spears-icon"
          v-if="attack.weaponGroup === 'Spears'"
        ></div>
        <div
          class="age-combat-section age-combat-staves-icon"
          v-if="attack.weaponGroup === 'Staves'"
        ></div>
        <div
          class="age-combat-section age-combat-magic-icon"
          v-if="attack.weaponType === 'Spell Ranged'"
        ></div>
        <div
          class="age-combat-section age-combat-magic-melee-icon"
          v-if="attack.weaponType === 'Spell Melee'"
        ></div>
        <div
          class="age-combat-section age-combat-pistols-icon"
          v-if="attack.weaponGroup === 'Pistols'"
        ></div>
        <div
          class="age-combat-section age-combat-longarms-icon"
          v-if="attack.weaponGroup === 'Long Arms'"
        ></div>
        <div
          class="age-combat-section age-combat-shotguns-icon"
          v-if="attack.weaponGroup === 'Shotguns'"
        ></div>
        <div
          class="age-combat-section age-combat-assaultrifles-icon"
          v-if="attack.weaponGroup === 'Assault Rifles'"
        ></div>
        <div
          class="age-combat-section age-combat-smgs-icon"
          v-if="attack.weaponGroup === 'SMGs'"
        ></div>
        <div
          class="age-combat-section age-combat-grenades-icon"
          v-if="attack.weaponGroup === 'Grenades'"
        ></div>
        <div
          class="age-combat-section age-item-weapon-icon"
          v-if="
            settings.gameSystem === 'expanse' && attack.weaponType === 'Melee'
          "
        ></div>
        <div
          class="age-combat-section age-item-weapon-ranged-icon"
          v-if="
            settings.gameSystem === 'expanse' && attack.weaponType === 'Ranged'
          "
        ></div>
      </div>
      <div
        class="age-combat-section age-combat-natural-icon"
        v-if="attack.weaponType === 'Natural'"
      ></div>
    </div>
    <div>
    ...
    <div class="age-weapon-range-reload age-combat-range">
      <div
        v-if="
          attack.weaponType === 'Ranged' || attack.weaponType === 'Spell Ranged'
        "
      >
    ...
    <div
      class="age-weapon-btn-container age-combat-damage"
      v-if="filteredDamageMods.length === 0"
    >
      <button class="age-btn" @click="handlePrintDamage(modsBonus)">
        {{ modsBonus }}
      </button>
    </div>
    <div
      ...
      <ul class="dropdown-menu">
        ...
        <div class="age-dmg-container" v-for="dm in damageMods" :key="dm">
          ...
          <button
            ...
            @click="
              handlePrintDamage(
                mergeDice([modsBonus, dm.roll ? dm.roll : dm.variable]),
                dm
              )
            "
          >
    ...
    <button
      type="button"
      class="config-btn age-icon-btn age-combat-details"
      @click="printAttackDetails(attack)"
      v-tippy="{ content: 'Share Attack in chat' }"
    >
    ...
    <div class="age-combat-config">
      <button
        type="button"
        v-if="attack.configurable"
        class="config-btn age-icon-btn"
        @click="showModal = true"
        v-tippy="{ content: 'Edit Attack' }"
      >
    ...
    <button
      class="accordion-button collapsed age-combat-accordion"
      type="button"
...
import {
  ...
  attackToHit,
  damageMod,
} from "@/sheet/stores/modifiersCheck/attackDamage";
...
const modsBonus = computed(() => {
  ...
  const [base, mod] = props.attack.damage
    ? props.attack.damage.split("+").map((part) => part.trim())
    : ["", ""];
  // useModifiersStore().modifiers = []
  const modifiersStore = useModifiersStore().modifiers.filter((item) => {
  ...
  if (settings.gameSystem === "fage2e" || settings.gameSystem === "blue rose") {
    if (props.attack.weaponType === "Spell Ranged") {
      abilityBonus.value = Number(
        ability.abilityScores[props.attack.weaponGroupAbility]?.base
      );
    } else {
      abilityBonus.value = Number(
...
const filteredDamageMods = computed(() => {
  return damageMods.value.filter((mod) => {
    ...
    if (
      props.attack.weaponType === "Melee" ||
      props.attack.weaponType === "Ranged"
    ) {
    ...
    if (
      props.attack.weaponType === "Spell Melee" ||
      props.attack.weaponType === "Spell Ranged"
    ) {
...
const setAttackRoll = () => {
  if (useAbilityScoreStore().abilityScores[props.attack.weaponGroupAbility]) {
    const attackAbility =
      props.attack.option === "Custom Attack" &&
      props.attack.weaponType === "Spell Ranged"
        ? "Accuracy"
        : props.attack.weaponGroupAbility;
    toAttackRoll =
      Number(useAbilityScoreStore().abilityScores[attackAbility].base) +
      trained.value;
  }
};
setAttackRoll();
...
const checkFocusBonus = (objectsArray, weapon) => {
  const obj = objectsArray.find((obj) => {
    if (obj.name === "custom") {
      ...
      return (
        obj.name.toLowerCase() === weapon.weaponGroup.toLowerCase() &&
        obj.weaponGroup === weapon.weaponGroup
      );
    }
  });
  if (obj) {
...
const printAttackDetails = (attack) => {
  const attackStore = useAttackStore();
  attackStore.printAttackDetails(
    props.attack,
    toAttackRoll,
    checkFocusBonus(useAbilityFocusesStore().abilityFocuses, props.attack)
  );
};
</script>
