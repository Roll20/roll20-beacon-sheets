<template>
  <div class="age-content">
      ...
      <div class="atk-section-header">
        <div class="age-search-input">
          <input
            ...
            placeholder="Search by Attack Name"
          />
          <button
    ...
    <div class="age-combat-footer">
      <div>
        <button
          class="age-label-black"
          @click="showWeaponGroupModal = true"
          v-if="
        ...
        >
          ...
          <div class="age-combat-wg">
            <div
              v-for="(wg, index) in char?.weaponGroups
                ? JSON.parse(char.weaponGroups).sort()
                : []"
              :key="wg"
            >
              {{ wg
              }}<span
                v-if="index != JSON.parse(char.weaponGroups).length - 1"
                class="wg-separator"
                >,
              </span>
            </div>
      ...
      <div class="age-combat-unarmed">
        <span
          class="age-flex"
          v-tippy="{
            content: (showUnarmed ? 'Hide' : 'Show') + ' Unarmed Attack',
          }"
        >
          ...
          <div class="age-checkbox-toggle">
            <label>
              <input
                type="checkbox"
                v-model="showUnarmed"
                @change="attackStore.handleShowUnarmed()"
              />
  ...
  <Teleport to="body">
    <WeaponGroupsModal
      :show="showWeaponGroupModal"
      @close="showWeaponGroupModal = false"
    >
      <template #header>
...
const weaponGroups = ref(
  char?.weaponGroups ? JSON.parse(char.weaponGroups) : []
);
const showUnarmed = attackStore.attacks.some(
  (atk) => atk.name === "Fist (Unarmed)"
);
const attackFilter = ref("");
let availableAttacks = ref([
  ...attackStore.attacks,
  ...useInventoryStore().items.filter(
...
const allAttacks = computed(() => {
  return [
    ...
    ...qualities.items.flatMap((item) =>
      (item.modifiers || [])
        .filter((modifier) => modifier.option === "Custom Attack")
        .map((modifier) => ({ ...modifier, description: item?.description }))
    ),
    ...mods.modifiers
      .filter((mod) => mod.option === "Custom Attack")
      .map((modifier) => ({
        ...modifier,
        description: qualities.items.filter(
          (itm) => itm._id === modifier.parentId
        )[0]?.description,
      })),
  ];
});
const modsBonus = computed(() => {
  return useModifiersStore()
    .modifiers.filter((item) => item.option === "Damage")
    .reduce((total, item) => {
...
const filteredAttacks = computed(() => {
  return allAttacks.value.filter((attack) => {
    ...
    const queryWords = debouncedSearchQuery.value
      ? debouncedSearchQuery.value.toLowerCase().split(" ")
      : [];
