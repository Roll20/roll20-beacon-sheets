<template>
  <Transition name="modal">
    <div v-if="show" class="modal-mask">
      <div
        class="modal-container age-modal age-qualities-modal-container"
        :class="{ 'age-qualities-modal': feature.type }"
      >
        ...
        <div class="modal-body">
          <div
            v-if="mode === 'create' && !feature.type"
            :style="`display: grid;grid-template-columns: repeat(${qualitiesLength}, minmax(min(100%, 30%), 1fr));gap: 1rem;`"
          >
            <button
              v-for="qty in qualityOptions"
              :key="qty"
              class="age-quality-select-btn"
              @click="
                feature.type = qty;
                if (qty === 'Favored Stunt') feature.spCost = '1';
              "
            >
              <div
                class="age-quality-section age-quality-class-icon"
                v-if="qty === 'Class'"
              ></div>
              <div
                class="age-quality-section age-quality-ancestry-icon"
                v-if="qty === 'Ancestry'"
              ></div>
              <div
                class="age-quality-section age-quality-focus-icon"
                v-if="qty === 'Ability Focus'"
              ></div>
              <div
                class="age-quality-section age-quality-talent-icon"
                v-if="qty === 'Talent'"
              ></div>
              <div
                class="age-quality-section age-quality-specialization-icon"
                v-if="qty === 'Specialization'"
              ></div>
              <div
                class="age-quality-section age-quality-stunt-icon"
                v-if="qty === 'Favored Stunt'"
              ></div>
              <div
                class="age-quality-section age-quality-special-icon"
                v-if="qty === 'Special Feature'"
              ></div>
              <div>
                {{ qty }}
              </div>
          ...
          <div>
            <div class="row age-row" v-if="feature.type === 'Ability Focus'">
              ...
              <div class="mb-3 col">
                ...
                <div class="custom-select-wrapper">
                  ...
                  >
                    ...
                    <option
                      v-for="foci in filteredFocuses[feature.ability]"
                      :key="foci"
                      :value="foci"
                    >
                      {{ foci }}
              ...
              <div class="age-focus-container">
                ...
                <div class="input-group">
                  <label
                    class="age-checkbox-toggle qualities-toggle-label"
                    v-tippy="{
                      content:
                        char.level < 10
                          ? 'Your character must be level 11 to double focus'
                          : '',
                    }"
                  >
                    <input
                      type="checkbox"
                      v-model="feature.doubleFocus"
                      :disabled="char.level < 10"
                    />
                    <span class="slider round"></span>
                  </label>
                  <span
                    class="age-toggle-label"
                    :class="{ 'age-toggle-label-disabled': char.level < 10 }"
                    >Double Focus</span
                  >
                </div>
                <!-- <div>
            ...
            <div
              class="row age-row"
              v-if="
                feature.type === 'Talent' || feature.type === 'Specialization'
              "
            >
              <div class="mb-3 col">
                <span class="age-input-label age-input-label" id="basic-addon1"
                  >Talent</span
                >
                <input
                  type="text"
            ...
            <div class="row age-row" v-if="feature.type === 'Favored Stunt'">
              ...
              <div class="mb-3 col-2">
                <span
                  class="age-input-label"
                  id="basic-addon1"
                  v-if="feature.type === 'Favored Stunt'"
                  >SP Cost</span
                >
                <input
                  type="text"
                  class="form-control"
                  placeholder="1"
                  aria-label="Stunt Point Cost"
                  v-model="feature.spCost"
                  aria-describedby="basic-addon1"
                />
              </div>
            </div>
            ...
            <div class="row age-row">
              <div class="mb-5 col" v-if="feature.type">
                ...
                <QuillEditor
                  ...
                  v-model:content="feature.description"
                />
              </div>
            </div>
            ...
            <div class="row age-row">
              <div
                class="mb-5 col"
                v-if="
                  feature.qualityLevel === 'novice' ||
                  feature.qualityLevel === 'expert' ||
                  feature.qualityLevel === 'master' ||
                  feature.qualityLevel === 'gransmaster' ||
                  feature.qualityLevel === 'apex'
                "
              >
                ...
                <QuillEditor
                  ...
                  v-model:content="feature.qualityNovice"
                />
              </div>
            </div>
            <div class="row age-row">
              ...
              >
                ...
                <QuillEditor
                  ...
                  v-model:content="feature.qualityExpert"
                />
              </div>
            </div>
            <div class="row age-row">
              ...
              >
                ...
                <QuillEditor
                  ...
                  v-model:content="feature.qualityMaster"
                />
              </div>
            </div>
            <div class="row age-row">
          ...
          <div
            class="age-quality-modifiers"
            v-if="
              feature.type &&
              feature.type !== 'Ability Focus' &&
          ...
          >
            ...
            <div
              v-for="mod in mode === 'create'
                ? feature.modifiers
                : mods.parentItems(feature._id)"
              :key="mod"
              class="qualities-mod-row"
            >
        ...
        <div class="modal-footer-actions" v-else>
          <slot name="footer">
            ...
            <div class="delete-container">
              <button
                class="delete-icon-btn delete"
                title="Delete"
                @click="
                  $emit('close');
                  $emit('delete');
                "
                v-tippy="{ content: 'Delete Ability' }"
              >
...
const arcanaFocuses = computed(() => {
  ...
  const sliceArcana = activeSlices().flatMap(
    (s) => sliceFocuses[s.key]?.Arcana || []
  );
...
const psychicFocuses = computed(() => {
  ...
  const slicePsychic = activeSlices().flatMap(
    (s) => sliceFocuses[s.key]?.Psychic || []
  );
...
const selected = ref(
  arcanaFocuses.value.includes(props.feature.name)
    ? `Arcana (${props.feature.name})`
    : psychicFocuses.value.includes(props.feature.name)
    ? `Psychic (${props.feature.name})`
    : props.feature.name || ""
);
...
const modOptions = computed(() => {
  ...
  const filtered =
    props.feature?.type === "Talent"
      ? base
      : base.filter((o) => o !== "Expertise");
