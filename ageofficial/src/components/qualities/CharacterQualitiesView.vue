<template>
  <div class="accordion-item">
    ...
    >
      <div class="qualities-icon-container">
        ...
        <div
          ...
          v-if="feature.type === 'Special Feature'"
        ></div>
      </div>
      <div class="label">
        <div v-if="feature.name !== 'custom'">{{ feature.name }}<br /></div>
        <div v-if="feature.name === 'custom'">
          {{ feature.customName }}<br />
        </div>
      ...
      <div class="age-weapon-range-reload">
        <div v-if="feature.type === 'Ability Focus'" class="age-cell-center">
          <div>
            <button
              ...
              "
            >
              <span v-if="feature.focus && !feature.doubleFocus"
                >+{{
                  Number(
                    useAbilityScoreStore().abilityScores[props.feature.ability]
                      ?.base
                  ) + focusBonus(feature)
                }}</span
              >
        ...
        <div v-if="feature.type === 'Favored Stunt'" class="age-cell-center">
          <div
            v-tippy="{
              content:
                char.stunts < getMinSpCost(String(feature.spCost || '1'))
        ...
        <div v-else-if="feature.type === 'Ancestry'" class="age-cell-center">
          <div v-for="mod in feature.modifiers" :key="mod">
            <div v-if="mod.option === 'Spell'">
              ...
              >
                <span
                  >{{ mod.customName }} - {{ mod.variable }}
                  {{ mod.costType === "Stunt Points" ? "SP" : "MP" }}</span
                >
              </button>
            </div>
          </div>
        ...
        <div
          class="age-orb-container"
          :class="{ 'age-adv-orb-container': settings.showAfterMastery }"
          v-if="
            feature.type !== 'Ability Focus' &&
            feature.type !== 'Favored Stunt' &&
        ...
        >
          <div
            ...
            :class="{
              'age-orb-highlight':
                isNovice || isExpert || isMaster || isGrandmaster || isApex,
            }"
            v-tippy="{ content: noviceTip }"
          ></div>
          <div
            class="age-orb"
            :class="{
              'age-orb-highlight':
                isExpert || isMaster || isGrandmaster || isApex,
            }"
            v-tippy="{ content: expertTip }"
          ></div>
          <div
            class="age-orb"
            :class="{
              'age-orb-highlight': isMaster || isGrandmaster || isApex,
            }"
            v-tippy="{ content: masterTip }"
          ></div>
          <div
            class="age-orb"
            :class="{ 'age-orb-highlight': isGrandmaster || isApex }"
            v-tippy="{ content: grandmasterTip }"
            v-if="settings.showAfterMastery"
          ></div>
          <div
            class="age-orb"
            :class="{ 'age-orb-highlight': isApex }"
            v-tippy="{ content: apexTip }"
            v-if="settings.showAfterMastery"
          ></div>
        </div>
      </div>
      <div class="age-weapon-btn-container"></div>
      <button
        type="button"
        class="config-btn age-icon-btn"
        @click="handlePrint"
        v-tippy="{ content: 'Share ' + feature.type + ' in chat' }"
      >
      ...
      <button
        ...
        :data-bs-target="
          '#collapseQuality' + feature.type.replace(' ', '') + index
        "
        aria-expanded="true"
    ...
    <div
      :id="'collapseQuality' + feature.type.replace(' ', '') + index"
  ...
  <Teleport to="body">
    ...
    <QualitiesModal
      ...
      :feature="feature"
      :index="index"
      @delete="handleDelete(feature.type)"
    >
      <template #header>
...
const handleStuntClick = () => {
  const options = parseSpOptions(
    String(props.feature.spCost || "1"),
    char.stunts
  );
  if (options.length === 1) {
    fireStunt(options[0]);
