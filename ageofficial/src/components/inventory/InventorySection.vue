      ...
      <div class="inv-body-top">
        ...
        <div class="inv-section-body">
          <div class="inv-section-header">
            ...
            <div class="inv-controls">
              <div class="age-search-input" v-if="inventory.items.length > 0">
                <input
                  class="form-control"
                  v-model="inventoryItemFilter"
                  placeholder="Search by Item Name"
                />
                <button
                  class="age-icon-btn age-search-input-clear-btn"
                  v-if="inventoryItemFilter"
                  @click="inventoryItemFilter = ''"
                >
            ...
            <button
              class="link-btn age-icon-btn inv-add-btn"
              @click="showModal = true"
              v-tippy="{ content: 'Add Equipment' }"
            >
          ...
          <div class="accordion age-accordion">
            ...
            <div
              v-if="filteredItems.length === 0 && inventory.items.length > 0"
              class="no-results"
            >
              No results found.
            </div>
...
const filteredItems = computed(() => {
  return inventory.items.filter((item) => {
    ...
    const matchesType =
      !debouncedItemType.value ||
      item.type.toLowerCase() === debouncedItemType.value;
