<template>
  <div class="accordion age-accordion">
    <div v-if="shipStore.qualityFlaws.length === 0" class="age-no-content-text">
      ...
      <button
        class="link-btn age-icon-btn qf-add-btn"
        @click="openNew"
    ...
    <template v-else>
      ...
      >
        <div class="accordion-header attack attack__row age-ship-qf-header">
          <div>
            <span>{{ qf.name }}</span>
            <span class="age-qf-badge" :class="qf.type">{{
              qf.type ? qf.type.charAt(0).toUpperCase() + qf.type.slice(1) : "—"
            }}</span>
          </div>
          ...
          <button
            ...
            :data-bs-target="'#collapseQF-' + index"
            aria-expanded="false"
            :aria-controls="'collapseQF-' + index"
          ></button>
        </div>
        <div
          :id="'collapseQF-' + index"
  ...
  <Teleport to="body">
    ...
    >
      <template #header>
        <h3 class="age-details-header">
          {{ modalMode === "new" ? "Add Quality/Flaw" : "Edit Quality/Flaw" }}
        </h3>
      </template>
      <template #body>
        ...
        <div class="row">
          <div class="mb-5 col">
            ...
            <QuillEditor
              ...
              contentType="html"
              toolbar=""
              v-model:content="qfObj.description"
            />
          </div>
        </div>
      </template>
...
async function sendQFToChat(qf) {
  await sendToChat({
    ...
    title: qf.name,
    subtitle: qf.type
      ? qf.type.charAt(0).toUpperCase() + qf.type.slice(1)
      : "Quality/Flaw",
