<template>
  <div
    ...
    :class="{ 'weapon--offline': props.systemOffline || props.weapon.offline }"
  >
    <div class="accordion-header attack attack__row age-ship-weapon-header">
      <div
        :class="
          'age-row-icon age-ship-weapon-' + (weapon.type || '').toLowerCase()
        "
      ></div>
      <div>
        <span>{{ weapon.name }}</span>
        <span v-if="props.systemOffline" class="age-offline-badge"
          >System Offline</span
        >
        <span v-else-if="weapon.offline" class="age-offline-badge"
          >Offline</span
        >
      </div>
      ...
      <div class="age-cell-center">
        <button
          ...
          v-tippy="{
            content: props.systemOffline
              ? 'System Offline'
              : weapon.offline
              ? 'Weapon Offline'
              : weapon.damage
              ? 'Roll ' + weapon.name + ' damage (' + weapon.damage + ')'
              : 'No damage formula set',
          }"
        >
      ...
      <button
        ...
        :data-bs-target="'#collapseQuality-weapon-' + index"
        aria-expanded="false"
        :aria-controls="'collapseQuality-weapon-' + index"
      ></button>
    </div>
    <div
      :id="'collapseQuality-weapon-' + index"
...
async function onRollDamage() {
  ...
  await rollToChat({
    ...
    traits: [
      w.range
        ? `Range: ${w.range.charAt(0).toUpperCase() + w.range.slice(1)}`
        : null,
    ].filter(Boolean),
    components,
  });
