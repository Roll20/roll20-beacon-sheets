<template>
  ...
  <div v-if="ship.crew.length === 0" class="age-no-crew-text">
    <h3>You have no crew!!</h3>
    <button
      class="link-btn age-icon-btn crew-add-btn"
      @click="openModal('new', 'crew')"
      v-tippy="{ content: 'Add Crew' }"
    >
  ...
  <div v-else>
    <h5 class="mt-3 age-section-relative">
      Members
      <button
        class="link-btn age-icon-btn age-section-action-btn"
        @click="openModal('options', null)"
        v-tippy="{ content: 'Add Crew' }"
      >
        <font-awesome-icon :icon="['fa', 'circle-plus']" />
      </button>
    ...
    <div class="accordion">
      <div
        v-for="(member, index) in ship.crew"
        :key="index"
        class="age-crew-member-entry"
        @click="openCrewEdit(member)"
      >
        <span class="age-crew-member-entry__name">{{ member.name }}</span>
        <div class="age-crew-member-entry__roles">
          <span
            v-for="primary of member.ability"
            :key="primary"
            class="age-crew-member-entry__role"
          >
            {{ primary.focusValue }}
          </span>
        </div>
        <div>
          <button
            type="button"
            class="ship-config-btn age-icon-btn"
            @click.stop="openCrewEdit(member)"
            v-tippy="{ content: 'Edit Crew Member' }"
          >
    ...
    <h5 class="mt-3 age-section-relative">
      ...
      <button
        class="link-btn age-icon-btn age-section-action-btn"
        @click="openStuntNew()"
        v-tippy="{ content: 'Add Crew Stunt' }"
      >
        <font-awesome-icon :icon="['fa', 'circle-plus']" />
      </button>
    </h5>
    ...
    <div v-else class="accordion age-accordion">
      <div
        v-for="(stunt, index) in ship.stuntDefs"
        :key="stunt._id"
      ...
      >
        <div class="accordion-header attack attack__row age-ship-stunt-header">
          <div>
            ...
            <span class="age-stunt-role-badge">{{
              roleLabel(stunt.role)
            }}</span>
          </div>
          <div
            class="age-cell-center"
            v-tippy="{
              content:
                ship.shipStunts[stunt.role] < stunt.sp
                  ? 'Not enough stunt points (' +
                    ship.shipStunts[stunt.role] +
                    ' available)'
                  : ship.shipStunts[stunt.role] + ' SP available',
            }"
          >
            <button
              class="age-btn"
              ...
              :class="{
                'age-btn-disabled': ship.shipStunts[stunt.role] < stunt.sp,
              }"
          ...
          <button
            ...
            :data-bs-target="'#collapseStunt-' + index"
            aria-expanded="false"
            :aria-controls="'collapseStunt-' + index"
          ></button>
        </div>
        <div
          :id="'collapseStunt-' + index"
          class="accordion-collapse age-accordion-collapse collapsed collapse"
        >
          <div class="accordion-body">
            <div class="age-quality-accordion">
              <span
                v-html="DOMPurify.sanitize(stunt.description || '—')"
              ></span>
            </div>
          </div>
        </div>
  ...
  <Teleport to="body">
    <ship-model-vue
      v-if="showModal"
      :show="showModal"
      :maxWidth="modalMode === 'options' ? 'fit-content' : '90dvw'"
      :type="modalText.type"
      @close="closeModal"
      @registerCrewMember="registerCrewMember"
    >
      <template #header>
      ...
      <template #body>
        <div class="age-ship-crew-options" v-if="modalMode === 'options'">
          <button
            v-for="cO in shipCrewOptions"
            :key="cO.value"
            class="age-quality-select-btn"
            @click="
              selectedModalOption = cO.value;
              modalMode = 'new';
            "
          >
        ...
        <div v-if="selectedModalOption === 'stunt'" class="row age-row">
          <div class="mb-3 col-12 col-sm-4 age-form-field">
            ...
            <select
              v-model="stuntEntry.role"
              class="age-atk-select form-select"
            >
              <option
                v-for="r in shipCrewRoles"
                :key="r.value"
                :value="r.value"
              >
                {{ r.label }}
          ...
          <div class="mb-5 col-12 age-form-field">
            ...
            <QuillEditor
              contentType="html"
              toolbar=""
              v-model:content="stuntEntry.description"
            />
          </div>
        </div>
        <div v-if="selectedModalOption === 'crew'" class="row age-row">
          ...
          <div
            v-for="(ability, ind) of crewMember.ability"
            :key="ind"
            class="row"
          >
            <div class="mb-3 col-12 col-sm-3 age-form-field">
              ...
              <div>
                <input
                  type="number"
                  class="form-control"
                  aria-label="Crew Member Ability Value"
                  v-model="crewMember.ability[ind].value"
                  aria-describedby="basic-addon1"
                />
              </div>
            </div>
            <div class="mb-3 col-5 col-sm-3 age-form-field">
              <span
                id="basic-addon1"
                class="age-input-label crew-ability-label"
              >
                {{ ability.ability }}
                ({{ ability.focusValue }})
              </span>
              <div class="input-group">
                <label class="age-checkbox-toggle age-checkbox-label">
                  <input
                    type="checkbox"
                    v-model="crewMember.ability[ind].focus"
                  />
            ...
            <div class="mb-3 col-7 col-sm-3 age-form-field">
              ...
              <div class="input-group">
                <label class="age-checkbox-toggle age-checkbox-label">
                  <input
                    type="checkbox"
                    v-model="crewMember.ability[ind].doubleFocus"
                  />
                  <span class="slider round"></span>
                </label>
            ...
            <div class="mb-3 col-7 col-sm-3 age-form-field">
              ...
              <div class="input-group">
                <label class="age-checkbox-toggle age-checkbox-label">
                  <input
                    type="checkbox"
                    v-model="crewMember.ability[ind].primary"
                  />
                  <span class="slider round"></span>
                </label>
        ...
        <div v-if="modalMode !== 'options'" class="modal-footer-actions">
          ...
          <div class="delete-container" v-if="modalMode === 'edit'">
            <button
              ...
              title="Delete"
              @click="deleteCrewEntry"
              v-tippy="{ content: 'Delete Crew' }"
            >
...
const modalText = computed(() => {
  if (selectedModalOption.value === "stunt") {
    return {
      type: "crew",
      header: modalMode.value === "new" ? "Add Stunt" : "Edit Stunt",
    };
  }
  if (modalMode.value === "options") return { type: "crew", header: "Add" };
  if (modalMode.value === "new")
    return { type: "crew", header: "Add Crew Member" };
...
const openCrewEdit = (member) => {
  crewMember.value = {
    ...member,
    ability: (member.ability || []).map((a) => structuredClone(a)),
  };
...
const updateSelected = (value) => {
  crewMember.value.ability = value.map((v) => {
    const existing = crewMember.value.ability.find(
      (a) => String(a._id) === String(v._id)
    );
    return (
      existing ?? {
        _id: String(v._id),
...
const registerCrew = () => {
  if (selectedModalOption.value === "stunt") {
    ...
    modalMode.value === "edit"
      ? ship.updateCrewMember(crewMember.value)
      : ship.addCrewMember(crewMember.value);
  }
  closeModal();
};
