<template>
  ...
  <div
    class="stunts-grid"
    @mouseenter="spHover = true"
    @mouseleave="spHover = false"
  >
      ...
      <div class="age-ship-container-content">
        <div class="stunts-input-row">
          ...
          <input
            ...
            type="number"
            v-model="ship.shipStunts[role.key]"
            min="0"
            autocomplete="off"
          />
    ...
    >
      <template #header>
        <h3 class="age-details-header">
          {{ modalMode === "new" ? "Add Stunt" : "Edit Stunt" }}
        </h3>
      </template>
      <template #body>
        <div class="row age-row">
          <div class="mb-3 col-12 col-sm-4 age-form-field">
            ...
            <select v-model="stuntObj.role" class="age-atk-select form-select">
              <option v-for="r of ROLES" :key="r.key" :value="r.key">
                {{ r.name }}
              </option>
            </select>
          </div>
...
const emptyStunt = (role = "commander") => ({
  _id: "",
  role,
  sp: 1,
  name: "",
...
const stuntsForRole = (roleKey) =>
  ship.stuntDefs.filter((s) => s.role === roleKey).sort((a, b) => a.sp - b.sp);
