<template>
  <div v-if="ship.weapons.length === 0" class="age-no-content-text">
    ...
    <button
      class="link-btn age-icon-btn weapon-add-btn"
      @click="openModal('new')"
  ...
  <div v-else class="weapons-list-wrapper">
    <button
      class="link-btn age-icon-btn age-section-action-btn weapons-add-floating"
      @click="openModal('new')"
      v-tippy="{ content: 'Add Weapon' }"
    >
      <font-awesome-icon :icon="['fa', 'circle-plus']" />
    ...
    >
      <template #header>
        <h3 class="age-details-header">
          {{ modalMode === "new" ? "Add Weapon" : "Edit Weapon" }}
        </h3>
      </template>
      <template #body>
        <div class="row age-row">
          ...
          <div class="mb-3 col-12 col-sm-6 age-form-field">
            ...
            <select
              v-model="weaponObj.range"
              class="age-atk-select form-select"
            >
              <option
                v-for="wr of weaponRanges"
                :value="wr.value"
                :key="wr.value"
              >
                {{ wr.label }}
          ...
          <div class="mb-3 col-12 col-sm-6 age-form-field">
            ...
            <input
              ...
              class="form-control"
              min="1"
              aria-label="Attacks Per Round"
              v-model.number="weaponObj.attacksPerRound"
            />
          </div>
        </div>
      </template>
