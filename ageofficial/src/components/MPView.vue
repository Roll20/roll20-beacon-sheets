<template>
  <div v-if="settings.showArcana">
    <div
      v-if="settings.userPowerFatigue"
      tabindex="0"
      class="mp-pf-container"
      @mouseenter="pfHover = true"
      @mouseleave="pfHover = false"
    >
      <div class="age-pf-cntr">
        ...
        <div class="age-pf-details">
          <div>
            <button
              class="age-icon-btn"
              v-show="char.powerFatigue > 0 && pfHover"
              @click="char.resetPowerFatigue"
              v-tippy="{ content: 'Click to reset to No Fatigue' }"
            >
          ...
          <div class="age-pf-manual-change" v-show="pfHover">
            <button
              class="age-icon-btn-sm"
              :disabled="char.powerFatigue > 3"
              @click="char.powerFatigue++"
              v-tippy="{ content: 'Manually increase Power Fatigue' }"
            >
              <font-awesome-icon :icon="['fa', 'caret-up']" />
            </button>
            <button
              class="age-icon-btn-sm"
              :disabled="char.powerFatigue === 0"
              @click="char.powerFatigue--"
              v-tippy="{ content: 'Manually decrease Power Fatigue' }"
            >
      ...
      <h4 class="age-subsection-header mp-pf-header">
        <span class="age-nowrap"> Power Fatigue </span>
        <button
          class="age-icon-btn mp-pf-dice-btn"
          v-if="pfHover"
          @click="showPFTestModal = true"
          v-tippy="{ content: 'Manually roll for Power Fatigue' }"
        >
    ...
    <div v-else>
      <div
        class="d-flex flex-column justify-content-between mp-bar-container"
      ...
      >
        ...
        >
          <div
            class="progress-bar"
            :class="{
              'bg-success': (char.magic / char.magicMax) * 100 >= 50,
              'bg-warning': (char.magic / char.magicMax) * 100 < 50,
              'bg-danger': (char.magic / char.magicMax) * 100 < 25,
            }"
            v-bind:style="{ width: (char.magic / char.magicMax) * 100 + '%' }"
          ></div>
        </div>
      </div>
      <div
        class="d-flex flex-column justify-content-between mp-bar-container mp-br-container"
        v-if="settings.gameSystem === 'blue rose'"
      >
        <div class="age-container-content">
          <button
            class="age-icon-btn age-reset-btn"
            v-if="char.stunts > 0"
            @click="char.resetStunts"
            v-tippy="{ content: 'Click to reset stunt points to 0' }"
          >
            <font-awesome-icon :icon="['fa', 'rotate']" />
          </button>
          <input
            class="age-input age-num-input"
            type="number"
            id="char.stunts"
            v-model="char.stunts"
            min="0"
  ...
  <Teleport to="body">
    ...
    <PowerFatigueModalView
      v-if="showPFTestModal"
      :show="showPFTestModal"
      @close="showPFTestModal = false"
    >
      <template #header>
