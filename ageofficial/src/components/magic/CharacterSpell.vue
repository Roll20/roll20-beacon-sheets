<template>
  <div class="accordion-header age-spell-accordion-header">
    <div class="spell-icon-wrapper">
      <div class="label spell-label-flex" data-testid="test-spell-header">
        {{ spell.name }}<br />({{ spell.requirements }})
      </div>
    </div>
    <div>
      <img
        v-if="spell.arcanaType && settings.gameSystem !== 'blue rose'"
        :src="'/src/assets/arcana/' + spell.arcanaType.toLowerCase() + '.png'"
        class="age-arcana-logo"
        v-tippy="{
          content: '<span>' + spell.arcanaType + ' ' + magicLabel + '</span>',
        }"
      />
    </div>
    ...
    <button
      ...
      :class="{
        'spell-btn-disabled': settings.userPowerFatigue
          ? false
          : char.magic < spell.mpCost,
      }"
    >
    ...
    <button
      type="button"
      class="config-btn age-icon-btn"
      @click="handleSpellDetails"
      v-tippy="{ content: 'Share ' + magicLabel + ' in chat' }"
    >
    ...
    <button
      ...
      v-tippy="{ content: 'Edit ' + magicLabel }"
    >
      ...
      <div class="age-spell-accordion">
        ...
        <div>
          <div class="age-spell-details">
            ...
            <span>{{ spell.targetNumber }}</span>
            <span class="age-spell-details__label" v-if="spell.spellTest"
              >Test</span
            >
            <span v-if="spell.spellTest">{{
              spell.spellTest + " vs. Spellpower"
            }}</span>
          </div>
        </div>
      </div>
  ...
  <Teleport to="body">
    <SpellFamiliarityModal
      :show="showFamiliarityModal"
      @close="showFamiliarityModal = false"
      :spell="spell"
      :index="index"
      :magicLabel="magicLabel"
