<template>
  <div class="section-card age-section-block">
    ...
    <h1 class="age-section-header">Actions</h1>
    <div class="age-actions-section">
      <div class="age-roll-initiative">
        <button
          class="age-roll-initiative-btn age-btn"
          @click="rollAbilityInitiative('Dexterity', true)"
        >
          <font-awesome-icon class="age-icon-right" :icon="['fa', 'dice']" />
          Roll for Initiative
        </button>
      </div>
  ...
  <Teleport to="body">
    <SidebarSection ref="sidebarRef" :className="'age-side-info'">
      ...
      <ul class="age-sidebar-list">
        <li v-for="(act, index) in actionsList" :key="index">
