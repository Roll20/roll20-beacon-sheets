<template>
  <div class="d-grid" :class="{ fortune: settings.useFortune }">
        ...
        <div class="d-flex flex-column justify-content-evenly hp-bar-container">
          ...
          >
            <div
              class="progress-bar"
              :class="{
                'bg-success': (char.health / char.healthMax) * 100 >= 50,
                'bg-warning': (char.health / char.healthMax) * 100 < 50,
                'bg-danger': (char.health / char.healthMax) * 100 < 25,
              }"
              v-bind:style="{
                width: (char.health / char.healthMax) * 100 + '%',
              }"
            ></div>
          </div>
        </div>
    ...
    <div
      ...
      class="d-flex flex-column justify-content-evenly age-fortune-btn-cntr"
    >
      <button
        :class="{ 'fortune-active': char.fortuneInjured }"
        class="age-fortune-btn"
        @click="char.fortuneInjured = !char.fortuneInjured"
        v-tippy="{ content: 'Injured' }"
      >
        <div class="age-fortune-injured-btn"></div>
      </button>
      <button
        :class="{ 'fortune-active': char.fortuneWounded }"
        class="age-fortune-btn"
        @click="char.fortuneWounded = !char.fortuneWounded"
        v-tippy="{ content: 'Wounded' }"
      >
