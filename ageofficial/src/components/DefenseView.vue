<template>
  <div class="section-card age-section-block">
    ...
    <h1 class="age-section-header">Defense</h1>
    <div class="age-actions-section">
      <div
        class="age-defense"
        :class="{
          'age-actions-magic-fage':
            settings.showArcana && settings.gameSystem === 'fage2e',
          'age-actions-magic-mage':
            settings.showArcana && settings.gameSystem === 'mage',
          'age-actions-magic-br':
            settings.showArcana && settings.gameSystem === 'blue rose',
        }"
      >
        ...
        <div class="age-container-content">
          <div class="age-container-heading">Defense</div>
        ...
        <div
          ...
          "
        >
          <div class="age-container-heading">Toughness</div>
        ...
        <div class="age-container-content" v-if="settings.showArcana">
          ...
          <span class="age-num-value">{{
            10 + Number(ability.WillpowerBase)
          }}</span>
          <!-- Additional corner elements -->
          <div class="age-container-content-corner-top-right"></div>
          <div class="age-container-content-corner-bottom-left"></div>
        ...
        <div class="age-container-content age-armor-stats">
          <div class="age-container-heading">Armor Rating | Penalty</div>
          <div class="age-flex">
            <span class="age-num-value">{{
              armorRatingSet !== null && armorRatingSet !== 0
                ? armorRatingSet
                : armorRatingMod
            }}</span>
            <span class="defense-separator">|</span>
            <span class="age-num-value">{{
              armorPenaltySet !== null && armorPenaltySet !== 0
                ? armorPenaltySet
                : armorPenaltyMod
            }}</span>
          </div>
...
import {
  armorRatingMod,
  armorRatingSet,
} from "@/sheet/stores/modifiersCheck/armorRating";
import {
  defenseMod,
  toughnessMod,
} from "@/sheet/stores/modifiersCheck/defense";
...
import {
  armorPenaltyMod,
  armorPenaltySet,
} from "@/sheet/stores/modifiersCheck/armorPenalty";
...
const filteredSections = computed(() => {
  return defenseList.filter((section) => {
    if (gameSystem.value === "fage") {
      return ["Defense", "Armor Rating", "Armor Penalty"].includes(
        section.label
      );
    } else if (gameSystem.value === "mage") {
      return ["Defense", "Toughness", "Armor Rating", "Armor Penalty"].includes(
        section.label
      );
    }
    return false;
...
const defense = computed(() => {
  return (
    ...
    (props.guard || props.guardOption === "always" ? props.guardValue : 0) +
    Number(defenseMods.value) +
    char.defenseLevelMod
  );
});
</script>
<style scoped>
