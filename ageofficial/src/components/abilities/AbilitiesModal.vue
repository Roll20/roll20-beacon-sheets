          ...
          <div v-for="score in abilityScoresArray" :key="score.label">
            <div class="mb-3 col">
              <span class="age-input-label ability-label-row" id="basic-addon1">
                {{ score.label }}
                <label
                  class="age-checkbox-toggle ability-toggle-compact"
                  v-tippy="{ content: 'Pending Ability Score Upgrade' }"
                >
              ...
              <input
                ...
                @change="
                  (evt) => updateScore(score.label, evt.target.value, 'base')
                "
              />
            </div>
          </div>
...
const abilityScoresArray = ref(
  Object.entries(abilityScores).map(([label, abilityScore]) => ({
    ...abilityScore,
    label,
  }))
);
