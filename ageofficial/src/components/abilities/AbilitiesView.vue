<template>
  ...
  <h1 class="age-section-header">Abilities</h1>
  <div
    class="age-ability-scores-container"
    :class="{ 'age-ability-scores-container-npc': !settings?.sheetView }"
  >
    <button
      @click="rollCheck(score.label, true)"
      class="age-abilities-btn"
      v-for="score in abilityScoresArray"
      :key="score.label"
    >
      <div class="age-container-content">
        ...
        <span class="age-num-value">
          {{ score.base }}
          <div
            class="age-ability-primary"
            v-if="
              (bio.profession === 'Envoy' &&
                (score.label === 'Communication' ||
                  score.label === 'Fighting' ||
                  score.label === 'Intelligence' ||
                  score.label === 'Willpower')) ||
              (bio.profession === 'Mage' &&
                (score.label === 'Accuracy' ||
                  score.label === 'Intelligence' ||
                  score.label === 'Perception' ||
                  score.label === 'Willpower')) ||
              (bio.profession === 'Rogue' &&
                (score.label === 'Accuracy' ||
                  score.label === 'Communication' ||
                  score.label === 'Dexterity' ||
                  score.label === 'Perception')) ||
              (bio.profession === 'Warrior' &&
                (score.label === 'Constitution' ||
                  score.label === 'Dexterity' ||
                  score.label === 'Fighting' ||
  ...
  <Teleport to="body">
    <AbilityCheckModal
      v-if="showAbilityCheckModal"
      :show="showAbilityCheckModal"
      :ability="selectedAbility"
      @close="closeCheckModal"
      @rerollCheck="onShowRerollModal"
    >
      <template #header>
  ...
  <Teleport to="body">
    <Transition name="modal">
      <div
        v-if="showRerollModal"
        class="snackbar-mask"
        :class="{ show: showRerollModal, hide: !showRerollModal }"
      >
        <div class="snackbar-container">
          <div class="snackbar-content">
            <p>
              You have a Talent or Specialization allowing you to reroll a
              failed test. You must accept the result of the second roll.
            </p>
            <div>
              <button
                class="ability-check-btn btn btn-danger"
                @click="showRerollModal = false"
              >
                Cancel
              </button>
            </div>
            <div>
              <button
                class="ability-check-btn btn btn-success"
                @click="
                  rollAbilityCheck(selectedAbility, true);
...
const rollCheck = (label, val) => {
  if (
    checkIfFocus(label) ||
    abilityMod.value.filter((am) => am.ability === label && am.name).length > 0
  ) {
...
const checkIfReRoll = (ability, focus) => {
  return quality.items.some(
    (foc) => foc.ability === ability && foc.modifiedOption === "Reroll"
  );
};
