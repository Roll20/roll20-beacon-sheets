<template>
  <div v-if="localNpc" class="npc-statblock" :class="{ 'npc-statblock--edit-mode': isEditing }" :data-npc-id="npc._id">
    <ImageUpload
...
const challengeRatingValue = computed({
  ...
  set: (cr) => {
    if (localNpc.value) {
      ...
      const pb = config.challengeRatingPB[cr as keyof typeof config.challengeRatingPB] || 2;
      localNpc.value.challenge = `${cr} (${xp.toLocaleString()} XP; PB +${pb})`;
    }
  },
});
...
const initiativeBonus = computed(() => {
  ...
  const keys = [effectKeys['initiative']];
  return store.getNpcModifiedValue(props.npc._id, dexterityBonus, keys);
});
...
const savingThrowsWithBonuses = computed(() => {
  ...
  const npcId = displayNpc.value._id;
  const npcEffects = store.getNpcEffects(npcId).value;

  return config.abilities.filter((key) => {
    if (displayNpc.value!.savingThrows[key] !== null) return true;

    if (npcId) {
      const bonus = store.getNpcSavingThrowBonus(npcId, key).value;
      if (bonus.modifiers.length > 0) return true;

      const keysToCheck = [
        effectKeys[`${key}-saving-roll` as keyof typeof effectKeys],
        effectKeys['saving-roll'],
        
        effectKeys[`${key}-saving-action-die` as keyof typeof effectKeys],
        effectKeys['saving-action-die'],
      ].filter(Boolean);

      const activeModifiers = EffectsCalculator.getValidEffects(
        npcEffects, 
        keysToCheck, 
        (e) => e.enabled
      );
...
const skillsWithBonuses = computed(() => {
  ...
  return (Object.keys(config.skills) as Array<keyof typeof config.skills>).filter(
    (key) => {
      if (displayNpc.value!.skills[key] !== null) return true;

      if (npcId) {
        const bonus = store.getNpcSkillBonus(npcId, key).value;
        if (bonus.modifiers.length > 0) return true;

        const ability = config.skills[key].ability;
        
        const keysToCheck = [
          effectKeys[`${key}-roll` as keyof typeof effectKeys],
          effectKeys['skill-roll'],
          effectKeys['check-roll'],
          effectKeys[`${ability}-check-roll` as keyof typeof effectKeys],
          
          effectKeys[`${key}-action-die` as keyof typeof effectKeys],
          effectKeys['skill-action-die'],
          effectKeys['check-action-die'],
          effectKeys[`${ability}-check-action-die` as keyof typeof effectKeys],
        ].filter(Boolean);
...
.npc-statblock {
  ...
  + .npc-statblock {
    border-top: 2px solid var(--sheet-border-color);
