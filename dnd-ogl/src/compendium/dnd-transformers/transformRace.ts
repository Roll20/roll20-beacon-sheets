...
export const transformDnDRace = (
  ...
  if (properties['data-datarecords']) {
    ...
    for (let i = parentRecords.length - 1; i >= 0; i--) {
      if (JSON.parse(parentRecords[i].payload).type === 'Species') {
        const children = childRecordsByParent[parentRecords[i].name] || [];
        parentRecords.splice(i, 1);
        for (const child of children) {
          if (parentRecordNames.has(child.name)) {
            parentRecords.push(child);
          } else {
            standaloneRecords.push(child);
          }
        }
      }
    }
