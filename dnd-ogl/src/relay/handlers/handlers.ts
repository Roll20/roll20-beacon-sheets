...
export const onInit = async ({ character, settings, compendiumDropData, sharedSettings }: InitArgs, dispatch: Dispatch) => {
  ...
  sharedSettingsRef.value = sharedSettings ?? {};
...
export const onSharedSettingsChange = (args?: SharedSettingsChangeArgs) => {
  if (!args) return;
  sharedSettingsRef.value = { ...sharedSettingsRef.value, ...args.settings };
};
