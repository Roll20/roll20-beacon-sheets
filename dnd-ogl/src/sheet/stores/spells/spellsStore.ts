...
export const SpellSchema = z.object({
  ...
  attackAbility: z.union([z.literal('none'), z.literal('spellcasting'), z.custom<AbilityKey>()]),
  critRange: z.number(),
  ...
  sourceEffectId: z.string().optional(),
  isFromEffect: z.boolean().optional(),
  effectId: z.string(),
  tagId: z.string(),
...
export const useSpellsStore = defineStore('spells', () => {
  ...
  const getEmptySpell = (patch: Partial<Spell>): Spell => {
    return {
      ...
      isAttack: false,
      attackType: 'melee',
      attackAbility: patch.isAttack === false ? 'none' : 'spellcasting',
      critRange: 20,
      damage: [],
      saving: 'none',
      description: { default: '' },
    ...
    userSources.value = payload?.sources
      ? objectToArray(payload.sources).map((source) => ({
          ...
          required: source.required
            ? indexedObjectToArray(source.required as unknown as Record<string, string>)
            : undefined,
        }))
      : userSources.value;
    ...
    if (isMulticlass) {
      if (effectPrefix === 'pact-spell-slots') {
        availableSlots = spellcasterClasses.reduce((acc, cls) => {
          if (config.casterTypes.hasOwnProperty('pact') && cls.spellcasting === 'pact') {
            const level = cls.level;
            const classSlots = config.casterTypes[cls.spellcasting as CasterType].slots[level - 1];
      ...
      if (effectPrefix === 'pact-spell-slots') {
        ...
        if (caster.spellcasting === 'pact' && config.casterTypes.hasOwnProperty('pact')) {
  ...
  const getStandardSlots = computed(() => {
    if (slots.value.mode === 'automatic') {
      return calculateSlots('spell-slots', (cls) => cls.spellcasting !== 'none' && cls.spellcasting !== 'pact');
    } else {
  ...
  const getSourceDC = (id: string): ModifiedValue => {
    ...
    if (source.type === 'flat') {
      return useEffectsStore().getModifiedValue(
        parseFormulaAndEvaluate(source.flat) + config.baseDC,
        effectKeys['spell-dc'],
      );
  ...
  const getSourceAttackBonus = (id: string): ModifiedValue => {
    ...
    if (source.type === 'flat') {
      return useEffectsStore().getModifiedValue(parseFormulaAndEvaluate(source.flat), [
  ...
  const getSpellAttackBonus = (spell: Spell): ModifiedValue => {
    const source = userSources.value.find((s) => s._id === spell.spellSourceId);
    let base = 0;

    const abilityKey = resolveAbility(spell.attackAbility, spell.spellSourceId, userSources.value);

    if (abilityKey !== 'none') {
      const ability = getEntryByLabel(abilityKey, useAbilitiesStore().abilities) as AbilityData;
      if (ability) {
        const mod = useAbilitiesStore().getAbilityModifier(ability).value.final;
        const pb = Math.floor(useProgressionStore().getProficiencyBonus);
        base = mod + pb;
      }
    } else {
      if ((spell.attackAbility === 'spellcasting' || !spell.attackAbility) && source?.type === 'flat') {
        base = source.flat;
      }
    }
    ...
    const keys = [
      effectKeys['spell-attack'],
      effectKeys['attack'],
      effectKeys[`${spell.attackType}-spell-attack`],
      effectKeys[`${spell.attackType}-attack`],
    ];

    return useEffectsStore().getModifiedValue(base, keys);
  };
  ...
  const getSpellDamage = (spell: Spell): ModifiedDicePool => {
    ...
    for (const damageRoll of spell.damage) {
      ...
      let abilityToUse: AbilityKey | 'none' | 'spellcasting' = damageRoll.ability;

      if (abilityToUse === 'spellcasting' && source && source.type === 'ability') {
        abilityToUse = source.ability;
      }

      if (abilityToUse !== 'none' && abilityToUse !== 'spellcasting') {
        const abilityModifier: ModifiedValue = useAbilitiesStore().getAbilityModifier(
          getEntryByLabel(abilityToUse as string, useAbilitiesStore().abilities) as AbilityData,
        );
        pool.value.push(abilityModifier.value.final);
      }
    }
