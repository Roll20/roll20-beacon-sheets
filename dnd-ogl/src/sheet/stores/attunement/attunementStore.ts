...
export const useAttunementStore = defineStore('attunement', () => {
  ...
  const hydrate = (payload: AttunementHydrate) => {
    ...
    const newMax = payload.attunement.maxSlots ?? 3;
    maxSlots.value = newMax;
    
    const hydratedIds = indexedObjectToArray(payload.attunement.attunedItemIds) ?? [];
    attunedItemIds.value = hydratedIds.slice(0, newMax);
  };
