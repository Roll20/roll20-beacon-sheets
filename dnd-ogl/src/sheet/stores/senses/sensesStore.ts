...
export const useSensesStore = defineStore('senses', () => {
  ...
  const hydrate = (payload: SensesHydrate) => {
    if (!payload?.senses?.manualSenses) return;
    
    config.senses.forEach((key) => {
      if (typeof payload.senses.manualSenses[key] === 'number') {
        manualSenses.value[key] = payload.senses.manualSenses[key];
      }
    });
  };
