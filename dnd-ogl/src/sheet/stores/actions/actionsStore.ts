...
export const useActionsStore = defineStore('actions', () => {
  ...
  const getActionSource = (action: Action) => {
    const spellsStore = useSpellsStore();
    if (action.spellSourceId) {
      return spellsStore.sources.find((s) => s._id === action.spellSourceId);
    }
    // Use the first available source if none specified but needed
    return spellsStore.sources[0];
  };
  ...
  const getActionBonus = (id: string): ModifiedValue => {
    ...
    if (action.attackAbility === 'spellcasting' && source?.type === 'flat') {
      baseBonus = source.flat;
    } else {
      const abilityKey = resolveAbility(
        action.attackAbility,
        action.spellSourceId,
        useSpellsStore().sources,
      );
      ...
      if (abilityKey !== 'none') {
        ...
        if (ability) {
          const abilityMod = useAbilitiesStore().getAbilityModifier(ability).value.final;
          const profBonus = Math.floor(
            useProgressionStore().getProficiencyBonus * action.attackProficiency,
          );
          baseBonus = abilityMod + profBonus;
        }
      }
    }

    baseBonus += parseFormulaAndEvaluate(action.attackBonus);
  ...
  const getActionDC = (id: string): ModifiedValue => {
    ...
    const action = actions.value[index];
    if (action.savingDc && action.savingDc.trim() === '@{spell-dc}') {
      const source = getActionSource(action);
      if (source) {
        return useSpellsStore().getSourceDC(source._id);
      }
    }
  ...
  const getActionDamage = (id: string): ModifiedDicePool => {
    ...
    for (const damageRoll of action.damage) {
      const abilityToUse = resolveAbility(
        damageRoll.ability,
        action.spellSourceId,
        spellsStore.sources,
      );
      ...
      const abilityModifier: ModifiedValue | boolean =
        abilityToUse !== 'none'
          ? useAbilitiesStore().getAbilityModifier(
              getEntryByLabel(abilityToUse, useAbilitiesStore().abilities) as AbilityData,
            )
          : false;
