  ...
  <div class="spell-view__spell-name">
    <details class="accordion">
      ...
      <div class="accordion__details">
        <p v-if="spell.school">
          <strong>{{ $t('titles.school') }}:</strong>
          {{ $t(`titles.spell-schools.${spell.school}`) }}
        </p>
...
const attackBonus = computed(() => spellsStore.getSpellAttackBonus(props.spell));
...
const rollArgs = computed((): D20RollArgs => {
  ...
  const abilityKey = resolveAbility(spell.attackAbility, spell.spellSourceId, spellsStore.sources);
  ...
  if (abilityKey !== 'none') {
    ...
    if (ability) {
      ...
      bonuses.push({ label: t(`titles.abilities.${abilityKey}`), value: rawAbilityBonus });
    ...
    const source = spellsStore.sources.find((s) => s._id === spell.spellSourceId);
    if (
      (spell.attackAbility === 'spellcasting' || !spell.attackAbility) &&
      source?.type === 'flat'
    ) {
  ...
  const modifiedValue = spellsStore.getSpellAttackBonus(spell);
...
const damageRollArgs = computed((): DamageRollArgs => {
  ...
  return {
    ...
    damageRolls: props.spell.damage.map((roll) => ({
      ...roll,
      ability: resolveAbility(roll.ability, props.spell.spellSourceId, spellsStore.sources),
      damage: evaluateDiceFormula(roll.damage),
      critDamage: roll.critDamage ? evaluateDiceFormula(roll.critDamage) : '',
      originalFormula: roll.damage,
    })),
