  ...
  <div class="view-container">
    <form ref="form" @submit.prevent="save">
      <div class="columns columns-4">
        ...
        <label class="span-2" v-if="usesSpellcasting">
          {{ $t('titles.spell-source') }}
          <select v-model="spellSourceId">
...
const spellSourceId = ref(actionCopy.value?.spellSourceId ?? spellsStore.sources[0]?._id ?? '');
...
const usesSpellcasting = computed(() => {
  if (attackAbility.value === 'spellcasting') return true;
  if (savingDc.value.includes('@{spell-dc}')) return true;
  if (damage.value.some(d => d.ability === 'spellcasting')) return true;
  return false;
});
