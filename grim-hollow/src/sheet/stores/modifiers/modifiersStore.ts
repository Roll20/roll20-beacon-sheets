...
export const useEffectsStore = defineStore('effects', () => {
  ...
  const getEmptyEffect = (patch: Partial<Effect>): Effect => {
    ...
    if (patch.actions)
      emptyEffect.actions = emptyEffect.actions?.map((item, i) => ({
        ...item,
        _id: item._id || (emptyEffect.actions?.[i]?._id ?? uuidv4()),
      }));
    if (patch.resources)
      emptyEffect.resources = emptyEffect.resources?.map((item, i) => ({
        ...item,
        _id: item._id || (emptyEffect.resources?.[i]?._id ?? uuidv4()),
      }));
    if (patch.spells)
      emptyEffect.spells = emptyEffect.spells?.map((item, i) => ({
        ...item,
        _id: item._id || (emptyEffect.spells?.[i]?._id ?? uuidv4()),
      }));
    if (patch.spellSources)
      emptyEffect.spellSources = emptyEffect.spellSources?.map((item, i) => ({
        ...item,
        _id: item._id || (emptyEffect.spellSources?.[i]?._id ?? uuidv4()),
      }));
    return emptyEffect;
  };
  ...
  const getModifiedProficiency = (
    ...
    computed(() => {
      const modifiedValue = getModifiedValue(base, attribute, proficiencyLevelsValues).value;
      return {
  ...
  const hydrate = (hydrateStore: EffectsHydrate) => {
    if (!hydrateStore?.effects) return;
    effects.value =
      objectToArray(hydrateStore.effects)?.map((effect) => ({
        ...
        spells: effect.spells
          ...
          : [],
        pickers: effect.pickers
          ? indexedObjectToArray(effect.pickers as Record<string, any>).map((picker: any) => ({
              ...picker,
