...
// Composant générique d'équipement, piloté par un store (PJ ou PNJ) exposant
// melee/dist/munitions/armor/equipment + add*/remove* + rollDamage.
...
const totalEqpWeight = computed(() =>
  props.inv.equipment.reduce((s, e) => s + (Number(e.qty) || 0) * (Number(e.weight) || 0), 0)
)
</script>
...
<template>
  <div class="equip">
    ...
    <section class="area">
      ...
      <div class="row" v-for="w in inv.melee" :key="w._id">
        <button class="roll-btn" title="Jet de dégâts" @click="inv.rollWeapon(w)">🎲</button>
        <button class="roll-btn whisper-btn" title="Jet privé (MJ)" @click="whisper(() => inv.rollWeapon(w))">🤫</button>
    ...
    <section class="area">
      ...
      <div class="row" v-for="w in inv.dist" :key="w._id">
        <button class="roll-btn" title="Jet de dégâts" @click="inv.rollWeapon(w)">🎲</button>
        <button class="roll-btn whisper-btn" title="Jet privé (MJ)" @click="whisper(() => inv.rollWeapon(w))">🤫</button>
        <label class="f-grow">Nom<input type="text" v-model="w.name" /></label>
        <label class="dmg">Dégâts
    ...
    <section class="area">
      ...
      <div class="row" v-for="w in inv.munitions" :key="w._id">
        <button class="roll-btn" title="Jet de dégâts" @click="inv.rollWeapon(w)">🎲</button>
        <button class="roll-btn whisper-btn" title="Jet privé (MJ)" @click="whisper(() => inv.rollWeapon(w))">🤫</button>
        <label class="f-grow">Nom<input type="text" v-model="w.name" /></label>
    ...
    <section class="area">
      ...
      <div class="total-weight">Poids total de l'équipement : <strong>{{ totalEqpWeight }}</strong></div>
    </section>
  </div>
</template>
