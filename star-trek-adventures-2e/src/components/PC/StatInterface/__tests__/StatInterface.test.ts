...
const doAttributeRender = ({ stat: attribute = 'CONTROL' }: AttributeRenderProps = {}) => {
...
const doDepartmentRender = ({ stat: department = 'COMMAND' }: DepartmentRenderProps = {}) => {
  const mounted = render(StatInterface, {
    props: {
      stat: department,
    },
...
describe('StatInterface', () => {
  ...
  it('should have a button in edit mode that changes the active edit modifier view', async () => {
  ...
  describe('Tooltip Interface', () => {
    it.todo(
      'should display a tooltip when the modifier button is hovered over or focused',
      async () => {
        const uiStore = useUIStore();
        uiStore.editMode = true;
        doAttributeRender();
    ...
    it.todo('should display an empty message when no modifiers are present', async () => {
      const uiStore = useUIStore();
      uiStore.editMode = true;
      doAttributeRender();
      const button = screen.getByRole('button');
      ...
      expect(entryNote.textContent).toBe(
        `No ${AttributesEnum.CONTROL} Modifiers. Click to add some!`,
      );
    });
  });
  ...
  describe('Attributes', () => {
    ...
    it.todo('should display each modifier for the attribute it represents', async () => {
      const statsStore = useStatsStore();
      ...
      modifierEntries.forEach((modifier, index) => {
        expect((modifier.children[0] as HTMLElement).textContent).toBe(
          `${getOperationSymbol(modifiers[index].operation)}${modifiers[index].value}`,
        );
        expect((modifier.children[1] as HTMLElement).textContent).toBe(`${modifiers[index].note}`);
      });
    });
  ...
  describe('Departments', () => {
    ...
    it.todo('should display each modifier for the department it represents', async () => {
      const statsStore = useStatsStore();
      const modifiers: StatModifier[] = [
        {
          value: 2,
